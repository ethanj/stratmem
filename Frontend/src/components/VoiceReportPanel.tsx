 ...
 * becomes a compact SALUTE JSON event when compression is enabled. It performs
...
const DEFAULT_AUDIO_ID = "raven_gap_salute_1";
...
export default function VoiceReportPanel({
  ...
  const audioId = voiceReport?.audio_id || DEFAULT_AUDIO_ID;
  ...
  return (
    <div className={`panel voice-report-panel ${status}`}>
      <div className="panel-header">
        <h2>VOICE REPORT</h2>
        <span>{comms?.kbps ? `${comms.kbps} KBPS LINK` : "DEGRADED LINK"}</span>
      </div>
