...
- Browser dashboard renders — landing overlay, hierarchy panel, live feed, demo-mode toggle.
...
If any of these fail, we go to Branch B.
...
|---|---|
| 0:00–0:05 | Press Replay → mesh tree visible, COP map ready, empty event stream |
...
- The cinematic landing overlay + `tecnet.mp4`
...
| Hosted LLM call for the final commander SITREP (heuristic fallback if it fails) | ~1h | Polish only — script-generated text is fine if LLM fails |

**Total: ~6–7 hours of tuning.** All in Python + vanilla JS — no React, no shadcn, no migration.
...
| Hour | Owner | Goal | Deliverable |
|---:|---|---|---|
| 1–3 | Backend lead | Replace `demo_loop()` with deterministic Raven Gap script | Script-driven replay; Replay button advances one event |
...
| 6–8 | Both | Hosted LLM brief generation + heuristic fallback | Brief renders <2s; falls back if API fails |
| 8–10 | All | **Hour-10 integration gate** — full 90s flow runs end-to-end |
| 10–12 | All | Polish: copy, contrast, projector readability | Looks intentional at 110% zoom |
| 12–14 | All | Bug fixes; rehearse demo with pitch lead | Three back-to-back full pitches |
