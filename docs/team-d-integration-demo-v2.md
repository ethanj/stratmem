# Team — D. Integration + Demo Infra + Laptop Operator (Generalist) — v2
...
**v2 judge-feedback change:** the demo must visibly prove operation at **3 Kbps degraded bandwidth**. You make that proof obvious on screen and verify it on the demo laptop.
...
- C's `DegradedCommsToggle` is a button/meter with `comms: any` and `onChange: (degraded: boolean) => void` props.
- C's `useSimulation` hook exposes a `toggleDegraded` action (calls `setCommsDegraded(degraded, 3)` from `services/api.ts`, then refreshes state).
- Your Dashboard renders `<DegradedCommsToggle comms={state.comms} onChange={toggleDegraded} />` and places it in the layout.
...
| **H5–7** | Static fallback fully tested with **DevTools network disabled**. Map still reads as a tactical surface (MGRS grid, NAIs, phase line all visible without tiles). This is not the bandwidth proof; it is only tile-fallback rehearsal. | **H7 sync with A+C**: end-to-end click-through on EW toggle. |
| **H7–9** | Active integration testing: click through the full 90-second flow, file bugs to A and C. Verify EW-degraded toggle changes map/feed detail and the 3 Kbps meter shows raw traffic over budget and compacted traffic fitting. Mount C's components in Dashboard. Wire evidence-click + degraded-toggle handshakes. | |
...
- [ ] 3 Kbps degraded-link proof verified in app: toggle degraded mode during rehearsal and confirm the UI shows raw traffic over budget and compacted traffic fitting.
