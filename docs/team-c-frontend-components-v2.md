# Team — C. Frontend Components (Generalist) — v2
...
**v2 judge-feedback change:** degraded bandwidth is now the visible proof point. The UI must show a **3 Kbps** budget where raw traffic exceeds the link and compacted traffic fits.
...
| `client/src/components/DegradedCommsToggle.tsx` + `client/src/styles/degraded.css` | Toggle plus compact bandwidth meter. Props: `comms: any`, `onChange: (degraded: boolean) => void`. D mounts and wires `onChange` to the `useSimulation` action. Shows `3 KBPS LINK`, raw bytes vs budget, compacted bytes vs budget, and compression ratio. |
...
| `client/src/services/api.ts` | **Add** `setCommsDegraded(degraded: boolean, kbps = 3)` wrapper for `POST /comms/degrade`. All API calls live here; do not hand-roll fetches in components. |
...
| **H5–7** | `EvidenceDrawer.tsx` + CSS done (click handler wired). `DegradedCommsToggle.tsx` + CSS done with visible 3 Kbps budget meter. `SitrepDeltaPanel.tsx` + CSS done. **`IncidentCard.tsx` extended** to render `incident.evidence_lines`. | |
...
**Degraded toggle:**
- You expose `<DegradedCommsToggle comms={...} onChange={...} />` and the `toggleDegraded` action on the `useSimulation` hook. The action calls `setCommsDegraded(degraded, 3)`.
...
- When off: show `FULL LINK`.
- When on: show `3 KBPS DEGRADED LINK`.
...
- Degraded toggle visibly shows `3 KBPS DEGRADED LINK`, raw traffic over budget, compacted traffic fitting, and compression ratio when `state.comms.degraded` is true.
