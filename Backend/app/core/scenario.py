...
RAVEN_GAP_EVENTS = [
    event(id="rg_001", type="physical.pli", source="1/A", domain="physical", severity="low", message="1/A crosses CP1; moving north on Route Finch.", metadata=raven_metadata(sender_id="1st_squad_team_a", unit_label="1st Squad Alpha Team", mesh_node="1st_squad", mgrs="11S LV 42181 49118", t_offset_sec=0, report_type="PLI", rollup_id="alpha", sequence=1, checkpoint="CP1", route="Route Finch", posture="moving"), geospatial={"lat": 37.4718, "lon": -118.6821}),
    event(id="rg_002", type="physical.lace", source="1/B", domain="physical", severity="medium", message="1/B LACE: leaders up, ammo amber, comms intermittent.", metadata=raven_metadata(sender_id="1st_squad_team_b", unit_label="1st Squad Bravo Team", mesh_node="1st_squad", mgrs="11S LV 42192 49142", t_offset_sec=15, report_type="LACE", rollup_id="alpha", sequence=2, leaders="green", ammo="amber", casualties="green", equipment="amber"), geospatial={"lat": 37.474, "lon": -118.6815}),
    event(id="rg_003", type="physical.salute", source="2/A", domain="physical", severity="medium", message="2/A SALUTE: two possible dismounts east ridge near NAI-2.", metadata=raven_metadata(sender_id="2nd_squad_team_a", unit_label="2nd Squad Alpha Team", mesh_node="2nd_squad", mgrs="11S LV 42274 49188", t_offset_sec=30, report_type="SALUTE", rollup_id="bravo", sequence=3, size="2 PAX", activity="observing route", nai="NAI-2"), geospatial={"lat": 37.4787, "lon": -118.6749}),
    event(id="rg_004", type="physical.sensor", source="OP-7", domain="physical", severity="medium", message="OP-7 reports acoustic trigger on east ridge draw.", metadata=raven_metadata(sender_id="sensor_s7", unit_label="OP/LP Sensor 7", mesh_node="op_lp", mgrs="11S LV 42306 49198", t_offset_sec=45, report_type="SENSOR", rollup_id="bravo", sequence=4, trigger="acoustic", nai="NAI-2", confidence=0.68), geospatial={"lat": 37.4796, "lon": -118.6715}),
    event(id="rg_005", type="physical.uas", source="RQ-11", domain="physical", severity="low", message="RQ-11 launches from CP1 to scan Route Finch ahead of PL Raven.", metadata=raven_metadata(sender_id="rq_11", unit_label="RQ-11 Raven", mesh_node="uas_team", mgrs="11S LV 42209 49130", t_offset_sec=60, report_type="UAS", rollup_id="alpha", sequence=5, asset="small UAS", task="route scan"), geospatial={"lat": 37.4729, "lon": -118.6798}),
    event(id="rg_006", type="physical.uas", source="RQ-11", domain="physical", severity="high", message="RQ-11 observes three dismounts moving south from NAI-2.", metadata=raven_metadata(sender_id="rq_11", unit_label="RQ-11 Raven", mesh_node="uas_team", mgrs="11S LV 42289 49203", t_offset_sec=75, report_type="UAS", rollup_id="bravo", sequence=6, asset="small UAS", nai="NAI-2", activity="3 dismounts moving south"), geospatial={"lat": 37.4801, "lon": -118.6734}),
    event(id="rg_007", type="physical.support", source="WPN", domain="physical", severity="low", message="WPN set support-by-observation west of Phase Line Raven.", metadata=raven_metadata(sender_id="weapons_squad", unit_label="Weapons Squad", mesh_node="weapons_squad", mgrs="11S LV 42147 49165", t_offset_sec=90, report_type="SUPPORT", rollup_id="charlie", sequence=7, position="support by observation", phase_line="PL Raven"), geospatial={"lat": 37.4762, "lon": -118.6858}),
    event(id="rg_008", type="physical.vehicle", source="JLTV-1", domain="physical", severity="medium", message="JLTV-1 delayed at CP2; culvert washout slows support movement.", metadata=raven_metadata(sender_id="jltv_v1", unit_label="JLTV-1", mesh_node="weapons_squad", mgrs="11S LV 42121 49083", t_offset_sec=105, report_type="ACE", rollup_id="charlie", sequence=8, checkpoint="CP2", equipment="amber", constraint="culvert washout"), geospatial={"lat": 37.4685, "lon": -118.6886}),
    event(id="rg_009", type="physical.pli", source="3/A", domain="physical", severity="low", message="3/A holds south spur; maintaining flank screen.", metadata=raven_metadata(sender_id="3rd_squad_team_a", unit_label="3rd Squad Alpha Team", mesh_node="3rd_squad", mgrs="11S LV 42220 49064", t_offset_sec=120, report_type="PLI", rollup_id="charlie", sequence=9, posture="holding", task="flank screen"), geospatial={"lat": 37.4669, "lon": -118.6775}),
    event(id="rg_010", type="physical.salute", source="2/B", domain="physical", severity="high", message="2/B confirms contact: three dismounts with handheld radio at NAI-2.", metadata=raven_metadata(sender_id="2nd_squad_team_b", unit_label="2nd Squad Bravo Team", mesh_node="2nd_squad", mgrs="11S LV 42286 49193", t_offset_sec=135, report_type="SALUTE", rollup_id="bravo", sequence=10, size="3 PAX", activity="moving south", equipment="handheld radio", nai="NAI-2"), geospatial={"lat": 37.4792, "lon": -118.6738}),
    event(id="rg_011", type="physical.ace", source="1/A", domain="physical", severity="medium", message="1/A ACE: one minor ankle injury; pace slowed but mission capable.", metadata=raven_metadata(sender_id="1st_squad_team_a", unit_label="1st Squad Alpha Team", mesh_node="1st_squad", mgrs="11S LV 42200 49173", t_offset_sec=150, report_type="ACE", rollup_id="alpha", sequence=11, ammo="amber", casualties="one minor", equipment="green", pace="slow"), geospatial={"lat": 37.4768, "lon": -118.6808}),
    event(id="rg_012", type="physical.command", source="PL", domain="physical", severity="high", message="PL updates NAI-2 risk high; collect with RQ-11 and keep squads west of PL Raven.", metadata=raven_metadata(sender_id="PL", unit_label="PL Raven", mesh_node="PL", mgrs="11S LV 42185 49159", t_offset_sec=165, report_type="SITREP_SEED", rollup_id="bravo", sequence=12, nai="NAI-2", risk="high", recommendation="maintain UAS collection"), geospatial={"lat": 37.4755, "lon": -118.6828}),
]
